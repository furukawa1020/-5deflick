...
拳で打つ場合は、認識を `マーカー` にして使うのがおすすめです。赤いテープやシールなど、盤面にない色を拳や手首に貼り、`マーカー` ボタンを押してからCamera映像上のその色をクリックします。以後はその色の中心が入ったキーと進入方向でフリック入力します。

`テスト` ボタンを押すと、認識を通さずOutputへ `あ` を入れます。ここで文字が出るなら出力経路は動いていて、問題はカメラ接続か認識側です。
